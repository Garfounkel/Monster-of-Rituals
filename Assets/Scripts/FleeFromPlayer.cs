...
public class FleeFromPlayer : MonoBehaviour {
    ...
    void Start () {
        ...
        _wallsRect = new Rect(transform.root.Find("BottomLeft").position, transform.root.Find("TopRight").position - transform.root.Find("BottomLeft").position);
    }
    ...
    private void Update()
    {
        ...
        var w = tr.x - bl.x;
        var h = tr.y - bl.y;
        var _wallsRect = new Rect(bl.x + marginH, bl.y + marginBottom, w - marginH*2, h - marginBottom - marginTop);*/
        /*var*/ //_wallsRect = transform.root.gameObject.GetComponent<RectTransform>().rect;
        /*var*/ //_wallsRect = new Rect(transform.root.Find("BottomLeft").position, transform.root.Find("TopRight").position - transform.root.Find("BottomLeft").position);
        //var bl = new Vector2(_wallsRect.xMin,_wallsRect.yMin);
        var bl = new Vector2(_wallsRect.xMin, _wallsRect.yMin);
        var tr = new Vector2(_wallsRect.xMax, _wallsRect.yMax);
        if (_player && Mathf.Abs(_player.transform.position.x - transform.position.x) < TriggerDistance && IsOnSameFloor())
        {
            if (_wallsRect.Contains(transform.position))
            {
            ...
            else
            {
                ...
                transform.position = Vector3.MoveTowards(transform.position, _wallsRect.center, 0.005f);
            }
        }
    }
    ...
    {
        if (transform.position.y > 4.5f && _player.transform.position.y > 4.5f) return true;
        if (transform.position.y < 4.5f && _player.transform.position.y < 4.5f) return true;
        return false;
    }
}
